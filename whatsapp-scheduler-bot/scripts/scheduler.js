...
async function enviarMensagem(destinatario, mensagem) {
    ...
    // ETAPA DE AQUECIMENTO: Se for um grupo, primeiro forçamos a API a carregar todos os grupos.
    if (isGroup) {
        console.log(`\n🔥 [GROUP_WARMUP] Destinatário é um grupo. Aquecendo cache da Evolution API...`);
        const warmupUrl = `${EVOLUTION_API_URL}/group/fetchAllGroups/${BOT_INSTANCE_NAME}?getParticipants=false`;
        try {
            const warmupController = new AbortController();
            const warmupTimeoutId = setTimeout(() => warmupController.abort(), 15000); // 15 segundos
            ...
            if (warmupResponse.ok) {
                console.log(`✅ [GROUP_WARMUP] Cache de grupos aquecido com sucesso!`);
            } else {
                const errorText = await warmupResponse.text();
                console.warn(`⚠️ [GROUP_WARMUP] Não foi possível aquecer o cache (status: ${warmupResponse.status}). O envio pode falhar. Erro:`, errorText);
            }
        } catch (error) {
            if (error.name === 'AbortError') {
                console.error(`❌ [GROUP_WARMUP] Timeout ao aquecer cache (15s)`);
            } else {
                console.error(`❌ [GROUP_WARMUP] Exceção ao tentar aquecer o cache:`, error.message);
            }
        }
    }
